...
def GetRecodeID(GenesID_RecodePair, OGFileSet, OG_GenesID_Pair):
	...
	for line in ReadFile(OGFileSet):
		...
		if id1 not in OG_GenesID_Pair.keys() or id2 not in OG_GenesID_Pair.keys() or id1 not in GenesID_RecodePair.keys() \
			...
			FNPair.append((recode1, recode2, label, statue))
	FNPairSplit = [FNPair[i:i + 1000] for i in range(0, len(FNPair), 1000)]
	done = time.time()
	return FNPairSplit


def IdentificationOGOfHHN(queue, hhn, FNs):
	PID = str(os.getpid())
	since = time.time()
	nodesEvents = []
	nodesPathEvents = []
	for fn in FNs:
		...
		if len(node1) > 0 and len(node2) > 0:
			...
			if shortest_path[0]:
				...
				nodesEvents.append('%s\t%s\t%d\t%d\t%f\n' % ('\t'.join(fn), LCAE, Total, Diff, DiffPro))
				nodesPathEvents.append('%s\t%s\t%s\t%s\n' % ('\t'.join(fn), LCAE, LCAEs1, LCAEs2))
			else:
...
def IdentificationOGOfHHNPP(hmFile, FNsList, Threads, fileName):
	pro = mp.Pool(int(Threads))
	MyQueue = mp.Manager().Queue()
	HHN = igraph.read(hmFile)
	# CCNumDic = FindConnectedComponents(HHN, NumThreads)
	for FN in FNsList:
		pro.apply_async(func=IdentificationOGOfHHN, args=(MyQueue, HHN, FN))
		# IdentificationOGOfHHN(MyQueue, HHN, FN)
...
if __name__ == '__main__':
	...
	IdentificationOGOfHHNPP(hm, FNPairs, threads, output)
